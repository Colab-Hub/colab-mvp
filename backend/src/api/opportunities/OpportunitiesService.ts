...
import {Opportunity} from "@/common/models/OpportunityModel";
...
export class OpportunitiesService {
    ...
    public async create(opportunity: Opportunity): Promise<ServiceResponse<Opportunity | null>> {
        try {
            const newOpportunityResponse: ServiceResponse<Opportunity | null> = await this.opportunitiesRepository.createAsync(opportunity);
            if (newOpportunityResponse.success) {
                const newOpportunity = newOpportunityResponse.responseObject;
                return ServiceResponse.success<Opportunity | null>("Opportunity created successfully", newOpportunity, StatusCodes.CREATED);
            } else {
                return ServiceResponse.failure<Opportunity | null>(
    ...
    public async update(opId: string, opportunity: Opportunity): Promise<ServiceResponse<Opportunity | null>> {
        try {
            const updatedOpportunityResponse: ServiceResponse<Opportunity | null> = await this.opportunitiesRepository.updateAsync(opId, opportunity);
